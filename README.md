...
### 2.5 1MB 오디오 파일 기준 실측 벤치마크 및 비교 (S20 vs A35)

실제 수집된 **1.0MB 용량의 음성 파일**(`음성_024_sd`)을 대상으로 동일한 **갤럭시 S20 기기**에서 두 가지 모델 설정 프로파일(초경량 구성 vs 고품질 구성)에 대한 STT + LLM 전처리 연산 소요 시간 및 품질을 계측한 결과입니다.

| 항목 | 초경량 구성 (Tiny + Qwen 0.5B) | 고품질 구성 (Small + Llama 3.2 3B) |
| :--- | :--- | :--- |
| **기기 사양** | Samsung Galaxy S20 (Snapdragon 865, 8GB RAM) | Samsung Galaxy S20 (Snapdragon 865, 8GB RAM) |
| **오디오 파일 크기** | 1.0 MB (`음성_024_sd_tiny.m4a`) | 1.0 MB (`음성_024_sd_small.m4a`) |
| **STT 엔진 / 모델** | Whisper.cpp (`ggml-tiny.bin` - 약 75MB) | Whisper.cpp (`ggml-small.bin` - 약 466MB) |
| **STT 소요 시간** | **10.0초** | **52.0초** |
| **LLM 엔진 / 모델** | Ollama (`qwen2.5:0.5b` - 약 350MB) | Ollama (`llama3.2:3b` - 약 2.0GB) |
| **LLM 소요 시간** | **11.0초** | **33.0초** |
| **총 소요 시간** | **21.0초** | **85.0초** |
| **메모리 점유 (추정)** | **~425 MB** (매우 안전, OOM 위험성 0%) | **~2.46 GB** (임계 치, OOM 및 스로틀링 위험 존재) |
| **요약 텍스트 정합성** | **기초적 요약** (어색한 띄어쓰기 및 종결어미 오타 일부 존재)<br>*예: "아프로 비슷한 상황이..."* | **우수한 요약** (자연스럽고 문맥에 완벽히 정합되는 요약 결과 도출) |

#### 📊 수학적/공학적 비교 분석 (동일 하드웨어 성능 분석)
1. **연산 속도 효율성 (Speed Up Factor)**:
   $$\text{Speed Up} = \frac{85.0\text{초}}{21.0\text{초}} \approx 4.05\times$$
   * 동일한 갤럭시 S20 하드웨어에서 초경량 모델 조합(Tiny + 0.5B)을 채택할 경우, 고품질 조합(Small + 3B) 대비 **약 4.05배 빠른 처리 속도**를 냅니다.
2. **자원 소모량 절감 및 안정성**:
   * **초경량 구성**: 메모리(VRAM) 점유율이 약 **425MB**에 불과하여 백그라운드 구동 시 시스템에 부하를 주지 않고 무중단 가동이 용이합니다.
   * **고품질 구성**: **약 2.46GB**의 고용량 메모리 사용으로 인해 장시간 가동 시 온디바이스 스로틀링(Throttling) 및 OOM(Out of Memory) 프로세스 강제 종료에 직면할 위험이 있어 세마포어 기반의 자원 격리/순차 실행 통제가 필수적입니다.
3. **요약 품질 트레이드오프**:
   * **Qwen 2.5 0.5B**는 가벼운 속도로 요약의 핵심 골자("대화 해결", "공통 목적")는 짚어냈으나 "아프로" 등 미세한 문법오차나 문장 매끄러움이 떨어집니다.
   * **Llama 3.2 3B**는 33초의 긴 시간이 걸리는 대신, 문장을 깔끔하고 완성도 높은 한국어로 논리 정연하게 서술하는 질적 성능 우위를 확연히 보여줍니다.
