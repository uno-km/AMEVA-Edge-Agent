...
| **LLM 소요 시간** | **11.0초** | **14.0초** | **33.0초** |
| **총 소요 시간** | **21.0초** (시스템 딜레이 포함 21초) | **25.0초** (시스템 딜레이 포함 32초) | **85.0초** (시스템 딜레이 포함 85초) |
...
1. **연산 속도 효율성 (Speed Up Factor)**:
   ...
   * **밸런스 vs 고품질**: $$\text{Speed Up} = \frac{85.0\text{초}}{32.0\text{초}} \approx 2.66\times$$ (밸런스 조합이 **약 2.6배 이상 고속**)
   * **초경량 vs 밸런스**: $$\text{Ratio} = \frac{32.0\text{초}}{21.0\text{초}} \approx 1.52\times$$ (밸런스가 약 1.5배의 시간만 더 할당하면 안정적인 텍스트 변환과 정밀한 요약 결과를 얻을 수 있음)
2. **자원 소모량 절감 및 안정성**:
